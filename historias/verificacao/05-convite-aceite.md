...
| Botão carrega `aviso_id` no payload (não número/token); webhook autenticado | [x] | botão `id` = `acao:avisoId`; parse valida UUID em `service.ts:42-55`; webhook HTTP da Meta com validação HMAC (`X-Hub-Signature-256`, `META_APP_SECRET`) | `convite_aceite.test.ts:86` (`b.id.endsWith(avisoId)`) |
...
| Critério | Status | Evidência | Teste |
|---|---|---|---|
| Webhook valida autenticidade (HMAC); payloads levam `aviso_id`, nunca número/token em claro | [x] | payload `acao:avisoId` validado como UUID (`service.ts:42-55`); webhook HTTP da Meta com validação HMAC (`X-Hub-Signature-256`, `META_APP_SECRET`) | `convite_aceite.test.ts:86` |
...
- H5.6 "webhook HMAC": o webhook HTTP da Meta com validação HMAC (`X-Hub-Signature-256`, `META_APP_SECRET`) já está implementado. O payload leva `aviso_id` (nunca número/token em claro), validado como UUID (`service.ts:42-55`). Critério atendido, classificado [x].
