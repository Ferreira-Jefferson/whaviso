...
- `[+]` Extração dos 6 dígitos da mensagem inicial: não existe (o inbound do zap, `shared/meta_client/inbound.ts`, só extrai clique de botão; texto livre é descartado).
...
- `[x]` `aviso_id` no payload do botão, canal autenticado: o padrão `acao:<avisoId>` já é o usado (`webhook_whatsapp/service.ts::parsearPayloadBotao`, valida UUID). HMAC: webhook HTTP da Meta com validação HMAC (`X-Hub-Signature-256`).
...
- **Captura de texto no transporte:** `shared/meta_client/inbound.ts` ganha `extrairTexto(m)` (texto da mensagem recebida via webhook da Meta), disparando handlers de texto além dos de botão. Acrescentar `onTexto(cb)`/`EventoTexto { telefone, texto, wamid }` ao contrato `ClienteWhats` (`tipos.ts`). Manter o transporte sem regra de negócio.
...
- Botão carrega `aviso_id` (UUID validado), nunca número/token. Canal autenticado por webhook HTTP da Meta com validação HMAC (`X-Hub-Signature-256`), conforme a história pede.
...
Objetivo: `onTexto`/`EventoTexto` no contrato e no `meta_client`; `extrairTexto`; disparo no processamento do webhook.
Arquivos: `shared/meta_client/{tipos.ts,inbound.ts,index.ts}`.
...
- **Resiliência de canal** (\_CONTEXTO §decisões): além dos botões interativos oficiais da Meta, prever **fallback numerado** (resposta por número/texto curto). Sinalizar como decisão (ver §7); o parser de texto de P5 já abre caminho para isso.
...
3. **Fallback de botões (resiliência de canal):** ligar já um fallback por resposta numerada/texto (1/2/3) como resiliência geral, ou deixar para depois? Afeta P5/P7.
