...
- **Canal/Baileys:** o envio usa `buttons` legacy (`type:1`) — instável em WhatsApp moderno. Risco real de os 3 botões não aparecerem. **Prever fallback** (resposta numerada no texto) — sinalizar como decisão de design (abaixo). Teste no mínimo do render (3 botões presentes); o fallback de canal é decisão de produto.
...
3. **Fallback de botões no Baileys:** os botões legacy podem não renderizar. Definir se o MVP já entra com fallback de resposta numerada no corpo da mensagem, ou se aceita o risco até a migração para a Meta Cloud API oficial.
