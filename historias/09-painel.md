...
- [ ] Há um destaque de **"precisa de você"** reunindo os combinados que aguardam minha ação: como cobrador, os em **`informado_pago`** ("aguardando sua confirmação") e sinalizações de **dado incorreto** (Épico 5); como qualquer papel, edições a aprovar quando aplicável.
...
- [ ] **Nada sensível** é exibido onde não deve nem fica em log: a **chave Pix** e o telefone seguem as regras (a chave aparece para quem é dono do dado, nunca em log).
...
- [ ] As ações disponíveis dependem do **estado** e do **meu papel**, e são as definidas nos épicos de origem. Exemplos:
  ...
  - `aguardando_aceite` (criador): **reenviar o combinado**, **editar** (livre), **cancelar** (Épicos 2/3/5).
...
- **"Precisa de você" (H9.2):** agregar combinados que aguardam ação do usuário (informado_pago, dado incorreto, edição a aprovar) é uma visão nova de pendências.
