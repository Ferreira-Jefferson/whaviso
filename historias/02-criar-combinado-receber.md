...
- **Sinal de "algum dado incorreto" no aceite:** terceira opção no aceite (além de aceitar/recusar), **sem texto livre**, que só notifica o criador para revisar. Mantido simples de propósito (não coleta o que mudar) para reduzir complexidade e risco. **Não cria status novo**; reusa a edição-antes-do-aceite (livre). Precisa de mensagens novas (notificação ao criador + resposta neutra ao convidado). Modelar no Épico 5.
  > **Nota (item 7, não confundir):** este sinal simples vale só **antes do aceite**. Depois do aceite, o combinado ativo (`programado`) tem um mecanismo **estruturado e separado** de reportar valor/data/nome ou motivo incorretos (com o que o devedor considera correto, aprovação do cobrador reabrindo a edição pré-preenchida e destacada), que cria um status novo próprio (`aguardando_aprovacao_dado_incorreto`) e uma tabela de reporte dedicada. Ver Épico 7 (H7.8/H7.9).
