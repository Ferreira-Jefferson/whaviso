...
*Critérios de aceite:*
- [ ] Há uma **lista central de clientes** (a outra ponta dos meus combinados, identidade pelo telefone, H15.1), agregada por número: o **número (completo, formatado)** como identidade e a **lista de nomes** registrados naquele número (sem eleger um nome único, pois varia por combinado), e os quatro totais da relação (H15.2). O número é dado do próprio dono (exibido a ele; nunca em rota/log, H15.7). Vive na área de Gestão (Épico 18, aba Clientes).
...
- [ ] O **cabeçalho** do modal mostra **só o número (formatado)** como identidade estável (H15.1): não há nome único no topo, porque o nome varia por combinado e não há como eleger o "certo". O renomear **não** fica no topo (o número é a identidade, não se renomeia); os nomes vivem nos grupos.
- [ ] Posso **editar o nome por grupo**: cada grupo de nome da lista (H15.3) tem seu próprio lápis. Salvar atualiza o `nome_devedor` **apenas dos combinados daquele grupo** (mesmo nome) em que sou cobrador, resolvendo o telefone **no servidor** a partir do id de combinado (nunca telefone em rota/log). Grupos em que sou só devedor não têm lápis (o renomear reescreve `nome_devedor`).
...
- [ ] Como a identidade é o telefone e o nome é só rótulo (H15.1), renomear afeta o rótulo, nunca o agrupamento: os combinados continuam sendo "da mesma pessoa" pelo número. Renomear um grupo para o nome de outro **funde** os dois grupos (passam a exibir o mesmo rótulo), sem mexer na chave (o número).
...
- **Lista central de clientes + edição do nome em modal (H15.8, 2026-07-16):** a gestão de clientes ganha uma lista (na área Gestão, Épico 18) e a edição do nome acontece em modal (escopada ao dono). Segue **sem tabela de contatos** (a identidade continua sendo o telefone); editar o nome só reescreve `nome_devedor` nos meus combinados daquele número.
- **Identidade é o NÚMERO na tela, nome vira lista (H15.8, revisado 2026-07-17):** como o nome varia por combinado e não há como eleger o "certo", o topo do modal e a linha da lista mostram **o número**; a lista traz ainda os **nomes registrados** naquele número (campo `nomes` do GET /v1/pessoas, mais recente primeiro, deduplicado). Não existe mais "nome mais recente" como título.
- **Renomear é POR GRUPO de nome (H15.8, revisado 2026-07-17):** o lápis vive em cada grupo de nome (não no topo, que mostra só o número), e salvar reescreve `nome_devedor` só nos combinados **daquele nome** onde sou cobrador (o corpo do PATCH leva `nome_atual` para escopar; ausente = número inteiro, retrocompat). Motivo: um número pode ter vários rótulos e o dono quer arrumar cada variação sem colapsar todas de uma vez (renomear um grupo para o nome de outro os funde de propósito).
