...
no Supabase Auth, que o POSTa em `POST /hooks/sms`. Aqui validamos a assinatura
(Standard Webhooks, `SEND_SMS_HOOK_SECRET = whsec_...`) e entregamos o código pelo
...
não-200 = falha (o usuário pede o código de novo). Sem `SEND_SMS_HOOK_SECRET` → `503`.

**Env:** `SEND_SMS_HOOK_SECRET` (opcional; sem ele a rota fica desligada).

O `zap` precisa estar **público** (a nuvem do Supabase chama o hook): VPS em prod,
ngrok em dev. A ENTREGA a +55 ainda é gated por verificação de empresa na Meta +
template de Autenticação aprovado; o código do fluxo já fica pronto.
