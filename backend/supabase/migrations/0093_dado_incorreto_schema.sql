-- E7 (item novo, plano 2026-07-22 grupo 1B), continuação da 0092: schema que USA os
-- valores novos de enum adicionados lá (status `aguardando_aprovacao_dado_incorreto` e
-- os 3 tipos de evento). Separado em arquivo/transação próprio porque Postgres não
-- permite usar um valor de enum recém-criado na MESMA transação em que foi adicionado
-- (ver comentário da 0092).
--
...
--
-- Numeração: última migration = 0092 (só os valores de enum); esta é 0093.

-- ---------------------------------------------------------------------------------------
-- 1) Tabela `avisos_reportes` (espelha avisos_edicoes, 0032): append-only, sem DELETE.
-- ---------------------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------------------
-- 2) Máquina de estados: acrescenta as transições de/para o novo estado (mesma forma de
...
-- ---------------------------------------------------------------------------------------
-- 3) Encerramento dos envios: o novo estado é de SUSPENSÃO (lembretes pausados enquanto
...
-- ---------------------------------------------------------------------------------------
-- 4) View combinado_linhas (0091, período de GET /avisos): o CASE de linha_status cai
